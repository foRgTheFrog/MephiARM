...
class NTAGReader:
    ...
    def read_with_authentication(self, key_hex=None, uid=None):
        """
        ...
        try:
            ...
            success, read_uid, all_data, user_data = self.rfid.read_ntag_with_key(key_hex, 'A', uid=uid)
            
            if not success or not read_uid:
                print("Не удалось прочитать метку")
                self.led.value(0)
                return False, None, None
            ...
            if all_data:
                ...
                if has_real_data:
                    self.rfid.print_ntag_info(read_uid, parsed_data)
                    ...
                    return True, read_uid, parsed_data
                else:
                    print("Метка пустая или данные не прочитаны")
                    
                    print("\nТестирование различных ключей...")
                    key_result = self.rfid.test_keys(read_uid)
    ...
    def auto_read(self):
        ...
        for key_hex in keys_to_try:
            ...
            success, read_uid, data = self.read_with_authentication(key_hex, uid=uid)
